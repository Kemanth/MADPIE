...
//Config::SetDefault ("ns3::PieQueueDisc::QueueDelayHard", TimeValue (Seconds (0.03)));
 ...
 /* 
  ...
  Simulator::Run ();
  PieQueueDisc::Stats st1 = StaticCast<PieQueueDisc> (queueDiscs.Get (0))->GetStats ();

  NS_TEST_EXPECT_MSG_NE (st1.unforcedDrop, 0, "There should be some packets dropped due to prob mark");
  NS_TEST_EXPECT_MSG_EQ (st1.forcedDrop, 0, "There should be zero packets dropped due to queue limit");
  NS_TEST_EXPECT_MSG_NE (st1.deterministicDrop, 0, "There should be zero packets dropped due to deterministic drop");
