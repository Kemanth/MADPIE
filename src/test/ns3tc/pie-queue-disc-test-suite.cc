...
private:
  ...
  void RunMADPieTest (StringValue mode);
...
PieQueueDiscTestCase::RunPieTest (StringValue mode)
{
  ...
  // test 2: PIE test in a simple dumbbell topology
  pktSize = 1000;
  ...
  */
  ...
  NS_TEST_EXPECT_MSG_EQ (st.deterministicDrop, 0, "There should be zero packets dropped due to deterministic drop");
...
void
PieQueueDiscTestCase::RunMADPieTest (StringValue mode)
{
  uint32_t pktSize = 0;
  // 1 for packets; pktSize for bytes
  uint32_t modeSize = 1;
  ...
  // test 1: MADPIE simple enqueue/dequeue with no drops
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("Mode", mode), true,
                         "Verify that we can actually set the attribute Mode");
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("QueueLimit", UintegerValue (qSize)), true,
                         "Verify that we can actually set the attribute QueueLimit");
  ...
  NS_TEST_EXPECT_MSG_EQ (queue->SetAttributeFailSafe ("QueueDelayHard", TimeValue (Seconds (0.03))), true,
                         "Verify that we can actually set the attribute QueueDelayHard");
  ...
  // test 2: MADPIE test in a simple dumbbell topology
  pktSize = 1000;
  std::string pieLinkDataRate = "1.5Mbps";
  std::string pieLinkDelay = "20ms";
  ...
  */
  ...
  NS_TEST_EXPECT_MSG_NE (st.unforcedDrop, 0, "There should be some packets dropped due to prob mark");
  NS_TEST_EXPECT_MSG_EQ (st.forcedDrop, 0, "There should be zero packets dropped due to queue limit");
  NS_TEST_EXPECT_MSG_NE (st.deterministicDrop, 0, "There should be some packets dropped due to deterministic drop");
...
PieQueueDiscTestCase::DoRun (void)
{
  ...
  RunMADPieTest (StringValue ("QUEUE_MODE_PACKETS"));
  RunMADPieTest (StringValue ("QUEUE_MODE_BYTES"));
  Simulator::Destroy ();
}
